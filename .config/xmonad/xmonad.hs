...
myBorderWidth        = 1
myNormalBorderColor  = "#5a524c"
myFocusedBorderColor = "#a9b665"
myWorkspaces         = ["1","2","3","4","5"]

myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    [ ((modm .|. shiftMask, xK_Return), spawn $ XMonad.terminal conf)
    , ((modm,               xK_c     ), spawn "rofi -show drun -theme gruvbox-dark")
    , ((modm .|. shiftMask, xK_c     ), kill)
    , ((modm,               xK_d     ), spawn "if pgrep polybar; then killall polybar; else polybar; fi")
    , ((modm,               xK_l     ), spawn "i3lock -c 000000")
    , ((modm .|. shiftMask, xK_l     ), spawn "i3lock -c 000000; sleep 2; systemctl suspend")
...
myLayout = smartSpacingWithEdge 10 $ mkToggle (single MIRROR) $ (noBorders Full ||| smartBorders tiled)
  where
    nmaster = 1
    ratio   = 1/2
    delta   = 3/100
...
myStartupHook = do
  spawnOnce "xrandr --output DP-2 --rotate right"
  spawnOnce "xrandr --output DP-4 --rotate left"
